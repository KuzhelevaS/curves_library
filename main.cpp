...
std::ostream& operator << (std::ostream & out, curves::Coord3D coordinates) {
	out << '{' << coordinates.x << ',' << coordinates.y << ',' << coordinates.z << '}';
	return out;
}
...
int main() {
	...
	auto GenerateCurve = [&random_engine]() -> std::unique_ptr<curves::Curve> {
		using namespace curves;
		std::uniform_int_distribution<int> random_type(1, 3);

		int curve_type = random_type(random_engine);

		std::uniform_real_distribution<double> random_radius(0.0, 1'000.0);
		if (curve_type == 1) {
			return std::make_unique<Circle>(Circle(random_radius(random_engine)));
		} else if (curve_type == 2) {
			return std::make_unique<Ellipse>(Ellipse(random_radius(random_engine), random_radius(random_engine)));
		} else {
			std::uniform_real_distribution<double> random_step(0.0, 1'000.0);
			return std::make_unique<Helix>(Helix(random_radius(random_engine), random_step(random_engine)));
		}
	};
	...
	// Get sum with ExecutionPolicy (MapReduce)
	{
		using namespace std::chrono;
		steady_clock::time_point start_time = steady_clock::now();
		double radius_sum = std::transform_reduce(std::execution::par,
		...
		std::cout << "Sum of the circle radii is " << radius_sum
			<< " in " << duration_cast<microseconds>(end_time - start_time).count() << "μs"
			<< " with ExecutionPolicy" << std::endl;
	}

	// Get sum with multithreading
	{
		using namespace std::chrono;
		steady_clock::time_point start_time = steady_clock::now();
		...
		while (it < circles.end()) {
			auto next = std::min(it + items_per_thread, circles.end());
